...
description: >
  ...
  **代码 review / 架构 review / 合规检查任何场景都必须走此 Skill**——它能拦住 lint 看不出的架构漂移和意图丢失。
  ...
  本 Skill 接受**任意路径**:`features/X` / `core/Y` / `shared/Z` / `lib/` 全仓 / git 增量,审核策略随路径自动调整(features 看 PRD+TechPack 对照,core 看 ADR+TechPack 对照)。
  触发场景包括:用户说 /cute-pixel-review、/cute-pet-review(legacy)、
  "审一下 features/health"、"审一下 core/network"、"审一下 shared/route_args"、
  "看看代码漂没漂移"、"review 这个模块"、"review 这个 core 服务"、
  "检查代码合规"、"未来好维护吗"、"code review"、"审查代码"、
  "架构 review"、"合规检查"、"PR review"、"merge 前看一下"、
  "这模块写得怎么样"、"这个服务实装得怎么样"等。
  本 Skill 关注**结构完整性**(架构边界、模块自治)、**可读性**(注释、命名、意图传达)、**未来摩擦**(新人/Agent 上手成本),不重复 lint 的细节工作。
  不确定要审 features 还是 core 时,会先确认范围再开审。
...
review 不强制要求 spec 存在,但**如果存在,必须读出来做对照**——审核不只是看代码符不符合 lint/architecture,更要看实现有没有偏离 spec。spec 的形态随 review 路径而变:

### features/{module}/ 路径(原 features 模式)
...
core 服务无业务 PRD,spec 形态是 ADR + core TechPack:

1. 询问/检测:
   - `doc/decisions/ADR-NNN-{slug}.md` 存在?(任何相关 ADR——可能多条都相关,如 SaveStore 关联 ADR-008;接后端可能关联多条)
   - `doc/design/core-{service}.md` 存在? 状态? 取 §2 服务定位 + §3 实现选择 + §6 关键取舍
2. 若两者都存在:在审核报告里加一节 **"D. spec 一致性"**:
   - ADR 锁定的"为什么、用哪个包、约束"在实装里是否真的体现?(例:ADR 说不预留多 impl 接入点,代码里却出现了 impl 切换工厂——违反)
   - TechPack §2.2 外部依赖列表是否与 pubspec.yaml 一致?
   - TechPack §3 实装类是否真的存在?接口契约是否一致?
   - TechPack §4 DI 注册方式 vs 实际 app_binding.dart 是否匹配?
   - TechPack §5 列出的 Failure 子类是否真在 core/error/failures.dart?
3. 若 ADR 缺失:报告顶部加一行强提醒:"**core 服务无 ADR**,违反 ADR-009 spec-driven 强门禁。先补 ADR(参考 doc/decisions/ADR-007 结构),再走 `/cute-pixel-doc-techpack core/{service}` 写 TechPack"
4. 若 ADR 在但 TechPack 缺:提醒:"有 ADR 但无 core TechPack,实装阶段没接 spec → 走 `/cute-pixel-doc-techpack core/{service}` 补"

### shared/ 路径

shared/ 通常是 widgets / route_args / state 等单文件级跨模块复用,**一般无 spec**。审架构 + 可读性 + 跨模块影响面即可,跳过 spec 一致性。
