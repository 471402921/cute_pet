...
## Spec → 代码 流水线(features 与 core 双轨,每步强门禁)

**features 流程**(业务模块):

```
/cute-pixel-doc-prd → /cute-pixel-doc-techpack(features) → /cute-pixel-module-gen → /cute-pixel-test-gen
   (起点,无门禁)        (PRD 必须定稿)                       (PRD+TechPack 都定稿)     (PRD §7 AC 必须定稿)
                                                                                              ↓
                                                                                  /cute-pixel-review (软门禁)
```

**core 流程**(基础设施服务,如 network/auth/env/logging):

```
ADR(doc/decisions/) → /cute-pixel-doc-techpack(core) → 手工实装 → /cute-pixel-review core/X
   (架构决策定锚)        (对应 ADR 必须存在)              (无 module-gen,core 异构无模板)
```
...
| `cute-pixel-doc-techpack`(features) | PRD 定稿 | 写 features TechPack(6 节,Module-First Flat),产 `doc/design/{NN}-{module}.md` | [SKILL.md](cute-pixel-doc-techpack/SKILL.md) |
| `cute-pixel-doc-techpack`(core) | 对应 ADR 存在 | 写 core 服务 TechPack(基础设施),产 `doc/design/core-{service}.md` | 同上,模式自动判断 |
| `cute-pixel-module-gen` | PRD + TechPack 定稿(features only) | `cp -r lib/features/_template/` + sed 起新模块 | [SKILL.md](cute-pixel-module-gen/SKILL.md) |
| `cute-pixel-test-gen` | PRD §7 AC 定稿 | 按 conventions §7 四层金字塔 + AC 写测试 | [SKILL.md](cute-pixel-test-gen/SKILL.md) |
| `cute-pixel-review` | 软(有 spec 则做对照) | 4 条铁律 + 12 条 conventions + spec 一致性,**任意路径**(features/core/shared),只报告 | [SKILL.md](cute-pixel-review/SKILL.md) |

**例外**:用户显式 `skip-spec: <原因>` 可越任何强门禁(prototype/throwaway 用),原因落进生成代码的 binding 注释 / TechPack 头部做 audit trail。
