...
make analyze             # check-all + flutter analyze(必须 0 issue / 0 violation)
make check-all           # check-arch + check-assets + check-arb-sync(三守门一起跑)
make check-arch          # 4 条铁律机械检查
make check-assets        # assets/_template/ 守门(模板不进 bundle、不放真资源)
make check-arb-sync      # zh/en ARB key 双语同步检查(conventions §4)
...
- [lib/shared/](lib/shared/) — 跨模块复用:`state/ViewState<T>`(in-use)、`widgets/StateViewBuilder`(in-use)、`route_args/{module}_route_args.dart`(路由参数统一住这,避免 features 互引)
