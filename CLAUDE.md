...
3. [doc/conventions.md](doc/conventions.md) — 11 条编码标准(P0:错误/环境/认证/i18n/日志/lint/测试;P1:路由/状态/JSON/跨模块)
4. [doc/README.md](doc/README.md) — `prd/` 与 `design/` 的写作流程

(根目录 [README.md](README.md) 是 `flutter create` 默认占位,无项目信息,不用读。)
...
- [lib/core/](lib/core/) — error / network / storage / utils(纯基础设施)
- [lib/shared/](lib/shared/) — 跨模块复用的 widget(`StateViewBuilder` 等)
- [lib/features/](lib/features/) — 业务模块,内部按下面 flat 结构
...
| Skill | 触发词 | 用途 |
|---|---|---|
| `/cute-pixel-module-gen` | "新建模块 X" / "起个 X feature" | 按 architecture + conventions 生成完整模块 |
| `/cute-pixel-review` | "review features/X" / "审一下" | 按 4 条铁律 + 11 条 conventions 扫违规(只报告) |
